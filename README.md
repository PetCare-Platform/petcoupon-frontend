...
시연용 이벤트·쿠폰은 프론트엔드 목데이터가 아니라 백엔드 API와 DB에 생성합니다. 백엔드 저장소 루트에서 다음 스크립트를 실행하면 이벤트 4개와 쿠폰 4개, Redis 재고가 생성됩니다.

```bash
./load-test/scripts/setup-demo-coupon.sh
```

생성 직후 이벤트는 `SCHEDULED` 상태이며, 기본 설정에서는 약 3분 뒤 `OPEN`으로 전환되어 프론트 이벤트 목록에 표시됩니다. 스크립트를 다시 실행하면 기존 데이터를 초기화하지 않고 새로운 이벤트와 쿠폰을 추가하므로 중복 실행에 주의합니다. 자세한 실행 조건과 옵션은 백엔드의 `load-test/README.md`를 확인합니다.
